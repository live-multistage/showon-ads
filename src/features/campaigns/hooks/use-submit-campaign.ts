...
// Submit chain per task 19: createAd (lands as DRAFT) → upload banner if one
// was staged → submit for review. If create succeeds but upload/submit fails,
// the ad already exists as a retryable DRAFT — redirect to its detail page
// (task 20) instead of silently losing it or retrying blindly here.
export function useSubmitCampaign() {
  ...
  const isSubmitting = createAd.isPending || uploadBanner.isPending || changeStatus.isPending;
